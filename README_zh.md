...
`tools/skill-recorder` 观察一段真实演示——你亲手操作，或由 FastCUA 自己驱动的合成演示——并把它编译成一份**可审计、不可执行**的技能草稿。整个流程由 Agent 驱动；用户负责演示、审查和拍板：

1. **录制** — 全局钩子 + UI Automation 锚点（数值控件类型 ID + AutomationId，本地化名称仅作提示）、稀疏 JPEG 关键帧（实测约 0.7 MB/分钟）、零依赖 **MJPEG 演示视频**（约 4 fps、长边 ≤1568，附 `video/index.jsonl` 支持按时间戳抽帧）、以及尽力而为的 **WAV 旁白轨**（WASAPI 录制，PCM 16 kHz 单声道；没有麦克风或被占用时只记一条说明，录制照常——媒体永远不会让会话失败）。`Ctrl+Alt+N` 输入旁白笔记、`Ctrl+Alt+R` 暂停、`Ctrl+Alt+X` 停止。密码框在钩子层结构性脱敏——对应视频帧被替换为带标记的黑帧缺口，绝不落盘像素；安全桌面被排除；录制器永不录制自己的窗口。所有内容只保存在本地会话目录。
2. **编译** — `session.jsonl` → `draft.json`/`draft.md` + 惰性的 `skill-draft/<name>/SKILL.md`（标记 `verified: false`，带双语「未验证草稿」横幅）。参数（日期、文件名、文本）**带来源**推断；一切不确定之处保留为显式 `⚠ unresolved` 标记。媒体仅以会话相对路径作为审查辅助引用（`draft.media`）——绝不嵌入草稿。
3. **审查** — Agent 在对话中呈现参数表、⚠ 标记、脱敏说明和已录制的应用范围；每一个未决步骤都由人来做决定。当某个步骤被质疑时，Agent 可以直接**查看**录制时刻：`frame-extract.mjs` 能把任意时间戳（或第 N 条旁白笔记）对应的视频帧抽成 JPEG；被脱敏的时刻会如实报告「此处无内容」。旁白 WAV 由用户自己回听（转写超出范围）。
4. **干跑（dry-run）** — 通过**正常控制平面**回放草稿（审批、白名单、F7–F10 全部生效）。未决步骤暂停等待显式决策；脱敏步骤永不执行；工作流永远不能超出录制时的应用范围；锚点无法重新解析时中止，而不是点错地方。每一步都记录期望与实际。干跑回放的是步骤，不是媒体。
5. **晋升** — 有闸门、由 Agent 执行、经用户批准。用户在对话中明确批准后，Agent 检测宿主的技能目录（Kimi Work、Claude Code、opencode，或显式 `--to`），然后运行 `promote.mjs`。缺少 `--yes-i-reviewed` 工具直接拒绝；`verified: false` 的草稿还需 `--force-unverified`，并会在晋升副本上追加额外警告。任何东西都不会被静默安装。
