...
| 等待确认 | 琥珀色 | 键盘 `1` 仅一次 · `2` 始终批准 · `3` 完全访问 · `4` 拒绝 |
...
**Agent 如何结束一次 FastCUA 任务？** 验证完成后调用一次 `close`。`close` 会结束当前回合并关闭该 MCP 客户端连接，但不会停止共享 daemon 或其他客户端。暂停只拦截新动作，不会结束任何进程。

**未知软件会静默启动吗？** 安全模式下不会。你可以仅允许一次、始终批准（白名单）、完全访问（关闭后续审批）或拒绝；审批浮层支持键盘 `1` / `2` / `3` / `4`。
