...
> **状态：已验证的预览版。** 在真实 Windows 11 上可用；已针对 FastCUA 测试夹具完成端到端验证（60+ 项真机检查：录制 → 编译 → 应用重启后用新参数值干跑 → 故障安全演练）。语音旁白属于后续工作；仍欠一次短时真人输入对比（目前所有验证输入均为自动化注入，并已被如实标记）。

`tools/skill-recorder` 观察一段真实演示——你亲手操作，或由 FastCUA 自己驱动的合成演示——并把它编译成一份**可审计、不可执行**的技能草稿：

1. **录制** — 全局钩子 + UI Automation 锚点（数值控件类型 ID + AutomationId，本地化名称仅作提示）、稀疏 JPEG 关键帧（实测约 0.7 MB/分钟）、`Ctrl+Alt+N` 输入旁白笔记、`Ctrl+Alt+R` 暂停、`Ctrl+Alt+X` 停止。密码框在钩子层结构性脱敏；安全桌面被排除；录制器永不录制自己的窗口。
2. **编译** — `session.jsonl` → `draft.json`/`draft.md` + 惰性的 `skill-draft/<name>/SKILL.md`（标记 `verified: false`，带双语「未验证草稿」横幅）。参数（日期、文件名、文本）**带来源**推断；一切不确定之处保留为显式 `⚠ unresolved` 标记。
3. **审查** — 草稿连同参数表、⚠ 标记、脱敏说明和已录制的应用范围一起呈现。每一个未决步骤都由人来做决定。
4. **干跑（dry-run）** — 通过**正常控制平面**回放草稿（审批、白名单、F7–F10 全部生效）。未决步骤暂停等待显式决策；脱敏步骤永不执行；工作流永远不能超出录制时的应用范围；锚点无法重新解析时中止，而不是点错地方。每一步都记录期望与实际。
5. **晋升** — 只有用户本人把审查通过、干跑干净的草稿复制进正式 skills 目录。草稿永不自我安装。
