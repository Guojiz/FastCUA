...
### 5. 视觉定位 = Apple 式正方形数字网格

**软件动作预算 30 秒**/次请求或 JS 单元。超时最多再试一次，然后换策略。

UIA 弱或 `state.uia.prefer_vision === true`（空树/壳树/点不到——见 [STUCK_zh.md](docs/STUCK_zh.md)）时：
...
Safe 模式对未知应用要求人工审批。信任匹配精确可执行路径/名，不用模糊子串。MCP 走命名管道；控制台只绑 `127.0.0.1`。策略留在本机。

### 8. Agent 中立，Skill 与 MCP 成对

不绑定某一家客户端。完整安装 = **同一个** 将要使用它的 Agent 里同时有 **Skill 目录** + **stdio MCP**。只装其一不算成功。
