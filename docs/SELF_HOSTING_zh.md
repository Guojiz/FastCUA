...
2. 把整个 `computer-use` 文件夹原样复制、链接或注册进去，不能改写内容。
3. 保留目录内的 `SKILL.md` 及全部同目录资源；不能只读取源文件，也不能创建指向源文件的转发版、包装版、摘要版或缩短版 `SKILL.md`。
4. 重新加载或重新索引 Skill。
5. 确认当前 Agent 可以发现并启用名为 `computer-use` 的 Skill。

只让 Agent 阅读仓库中的 `SKILL.md`，或创建一个指回源文件的壳，都不算安装。使用文件夹式 Skill 系统时，应比较安装前后的相对文件列表和 SHA-256；使用原生 Skill 注册机制时，应确认活动注册实际解析到完整目录。
